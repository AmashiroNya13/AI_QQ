...
fn startup_error_script(message: &str) -> String {
    let message_json =
        serde_json::to_string(message).unwrap_or_else(|_| "\"Ecobot startup failed.\"".to_string());
    format!(
    ...
    F: Fn(&str),
{
    ...
    eprintln!("Ecobot startup failed: {message}");
    let Some(window) = app_handle.get_webview_window("main") else {
        log("failed to display startup error: main window not found");
        app_handle.exit(1);
        return;
    };
    if let Err(error) = window.set_title("Ecobot - 启动失败 / Startup failed") {
        log(&format!(
