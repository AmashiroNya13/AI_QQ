...
const ECOBOT_UPDATER_ENABLED_ENV: &str = "ECOBOT_DESKTOP_UPDATER_ENABLED";
...
pub(crate) fn resolve_desktop_update_mode() -> DesktopUpdateMode {
    if std::env::var(ECOBOT_UPDATER_ENABLED_ENV).as_deref() != Ok("1") {
        return DesktopUpdateMode::Unsupported;
    }
