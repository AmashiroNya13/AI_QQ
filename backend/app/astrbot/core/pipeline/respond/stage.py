...
class RespondStage(Stage):
    ...
    async def process(
        ...
        if result is None:
            return
        ecobot_bridge = event.get_extra("_ecobot_behavior_bridge")
        ecobot_batch_id = event.get_extra("_ecobot_expression_batch_id")
        delivery_attempted = False
        delivery_failed = False
        delivery_error = None
        delivery_marked = False

        def mark_ecobot_delivery(success: bool, error: str | None = None) -> None:
            nonlocal delivery_marked
            if delivery_marked or ecobot_bridge is None or not ecobot_batch_id:
                return
            ecobot_bridge.mark_expression(ecobot_batch_id, success=success, error=error)
            delivery_marked = True
        ...
        if event.get_extra("_streaming_finished", False):
            ...
            mark_ecobot_delivery(False, "streaming result already finished")
            return
        ...
        if (
            ...
            mark_ecobot_delivery(False, "message chain was empty")
            return
        ...
        if (
            ...
            mark_ecobot_delivery(False, "message was already delivered by a tool")
            return
        ...
        if result.result_content_type == ResultContentType.STREAMING_RESULT:
            if result.async_stream is None:
                ...
                mark_ecobot_delivery(False, "streaming result had no stream")
                return
            # 流式结果直接交付平台适配器处理
            realtime_segmenting = (
            ...
            try:
                ...
                mark_ecobot_delivery(not delivery_failed, delivery_error)
            return
        if len(result.chain) > 0:
            ...
            if self.is_seg_reply_required(event):
                ...
                if not result.chain or len(result.chain) == 0:
                    ...
                    mark_ecobot_delivery(False, "message chain became empty after decoration")
                    return
                for comp in result.chain:
                ...
                if all(
                    ...
                    mark_ecobot_delivery(False, "message chain contained only reply headers")
                    return
        ...
        if ecobot_bridge is not None and ecobot_batch_id:
            success = delivery_attempted and not delivery_failed
            error = delivery_error or (None if success else "message was not delivered")
            mark_ecobot_delivery(success, error)
            if ecobot_bridge.qq_archive is not None:
                ecobot_bridge.qq_archive.record_outbound_message(
                    channel_id=event.unified_msg_origin,
                    content=result.get_plain_text(),
                    batch_id=ecobot_batch_id,
                    success=success,
                    error=error,
                    platform_name=event.get_platform_name(),
                    group_id=event.get_group_id(),
                    target_qq_id=(event.get_sender_id() if event.is_private_chat() else None),
                )
