    ...
    def _split_plain_text(self, text: str) -> list[str]:
        ...
        for paragraph in paragraphs:
            ...
            if self.split_mode == "words":
                result.extend(self._split_text_by_words(paragraph))
                continue
            try:
                segments = re.findall(
                    self.regex,
                ...
                logger.error(
                    "Invalid segmented-reply regular expression; using the default segmentation method: "
            ...
            result.extend(str(segment) for segment in segments if str(segment).strip())
        return result or [text]

    @staticmethod
    def _limit_segments(segments: list[str], maximum: int = 5) -> list[str]:
        ...
        if len(result.chain) > 0:
            ...
            if self.enable_segmented_reply and event.get_platform_name() not in [
                ...
                if (
                    ...
                    for comp in result.chain:
                        if isinstance(comp, Plain):
                            split_response = self._split_plain_text(comp.text)
                            if event.get_extra("ecobot_behavior_result") is not None:
                                split_response = self._limit_segments(split_response, 5)
